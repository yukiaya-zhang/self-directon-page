...
一个为个人日常使用设计的静态导航页，使用原生 `HTML + CSS + JavaScript` 实现，可直接下载从本地打开，也可从发布网页开启（github网页可能会获得更好的用户体验在未来的更新中）。
...
- [index.html]：页面骨架和各类弹层节点
- [styles.css]：整体视觉、响应式和组件样式
- [js/app.js]：页面交互、渲染、天气、主题和设置逻辑
- [js/store.js]：状态模型与纯逻辑存储函数
- [js/store.browser.js]：浏览器直开场景使用的存储实现
- [tests/store.test.js]：状态层回归测试
